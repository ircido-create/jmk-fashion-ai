...
Deno.serve(async (req) => {
  ...
  // 1. Get Doiane's phone
  const { data: cust } = await supabase.from("customers").select("id, name, phone").eq("name", "DOIANE PEREIRA SANTOS").maybeSingle();
  ...
  if (!overdue) return new Response("No overdue debt found for Doiane", { status: 404 });
  ...
  if (res.ok) {
    ...
    return new Response(JSON.stringify({ ok: true, message: "Dunning sent manually for Doiane", bw: resText }), { headers: { "Content-Type": "application/json" } });
  } else {
